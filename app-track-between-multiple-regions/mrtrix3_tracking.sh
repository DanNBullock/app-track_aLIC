...
mrconvert gmwmi_seed.mif gmwmi_seed.nii.gz -force -nthreads $NCORE -quiet
...
if [ $MS -eq 0 ]; then
	...
	#time dwi2fod -mask ${mask}.mif csd ${difm}.mif wmt.txt wmt_lmax${lmax}_fod.mif -lmax $lmax -force -nthreads $NCORE 
	time dwi2fod  msmt_csd ${difm}.mif wmt.txt wmt_lmax${lmax}_fod.mif csf.txt csf_lmax${lmax}_fod.mif -mask ${mask}.mif -lmax $lmax,$lmax -force -nthreads $NCORE 
    ...
    #mtnormalise -mask ${mask}.mif wmt_lmax${lmax}_fod.mif wmt_lmax${lmax}_norm.mif  csf_lmax${lmax}_fod.mif csf_lmax${lmax}_norm.mif -force -nthreads $NCORE 
...
for file in output/ROIS/ROI_*.nii.gz
do
    ROIS2Track=("${ROIS2Track[@]}" "$file")
done

#go ahead and convert the aLIC rois
mrconvert output/ROIS/fullCutIC11_ROI_right.nii.gz output/ROIS/fullCutIC11_ROI_right.mif -force -nthreads $NCORE -quiet
mrconvert output/ROIS/fullCutIC11_ROI_left.nii.gz output/ROIS/fullCutIC11_ROI_left.mif -force -nthreads $NCORE -quiet
...
divisor=2
iterationsNum=$((${trackROISNuM} / $divisor))

#toIterate=  $(1..$iterationsNum)
#echo $toIterate

for iterations in $(seq $iterationsNum); do
    echo $iterations
    #get index 1
    sourceIndex=$((($iterations * 2)-1))
    #get index 2
    targetIndex=$((($iterations * 2)))
    #source
    mrconvert output/ROIS/ROI_${sourceIndex}.nii.gz output/ROIS/ROI_${sourceIndex}.mif -force -nthreads $NCORE -quiet
    #target
    mrconvert output/ROIS/ROI_${targetIndex}.nii.gz output/ROIS/ROI_${targetIndex}.mif -force -nthreads $NCORE -quiet
    ...
    tckedit wb*.tck track${iterations}.tck -force -nthreads $NCORE -quiet

    ## find the final size
    COUNT=`tckinfo track${iterations}.tck | grep -w 'count' | awk '{print $2}'`
    ...
    ## simple summary text
    tckinfo track${iterations}.tck > tckinfo${iterations}.txt
done
##
